...
const ADDITIONAL_EXCLUDE = ["**/.stack-work/**", "node_modules/**"]
...
class ServerlessPlugin {
  constructor(serverless, options) {
    ...
    this.hooks = {
      ...
      "after:package:createDeploymentArtifacts": this.cleanupHandlers.bind(
        this
      ),
  ...
  runStack(directory, args, options) {
    ...
    const stackArgs = [...envArgs, ...this.custom().stackBuildArgs, ...args]
    ...
    if (result.error || result.status > 0) {
      const message =
        `Error when running Stack: ${result.stderr}\n` +
  ...
  dependentLibraries(directory, executablePath) {
    try {
      const lddOutput = this.runStackOutput(directory, [
        "exec",
        "ldd",
        executablePath
      ])
  ...
  assertServerlessPackageVersionsMatch(directory) {
    ...
    const stackDependencies = this.runStackOutput(directory, [
      "ls",
      "dependencies"
    ]).split("\n")
    const haskellPackageVersions = stackDependencies.filter(dep =>
      dep.startsWith(`${PACKAGE_NAME} `)
    )
    if (haskellPackageVersions.length === 0) {
      this.serverless.cli.log(
        `Could not find ${PACKAGE_NAME} in stack's dependencies. Make sure ${PACKAGE_NAME} you are using LTS 12 (or newer), or add it as an extra-dep in your stack.yaml, and reference it in package.yaml or the Cabal file.`
      )
  ...
  addToHandlerOptions(
    ...
    handlerOptions[directory][packageName] =
      handlerOptions[directory][packageName] || []
    handlerOptions[directory][packageName].push([
      executableName,
      {
